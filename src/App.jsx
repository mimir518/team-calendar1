...
const STORAGE_KEY = 'team-calendar-overrides-2026';
...
function buildInitialOverrides() {
  return {
    '2026-03-10': { Doris: 'off', Mary: 'ho-am' },
    '2026-03-11': { Doris: 'off', Mark: 'business', Helen: 'ho-pm' },
    '2026-03-12': { Mark: 'business', Jerry: 'ho' },
    '2026-03-13': { Helen: 'off-am' },
    '2026-06-18': { Mary: 'off' },
    '2026-06-19': { Mary: 'off-pm', Jerry: 'business' },
    '2026-08-04': { Doris: 'business' },
    '2026-08-05': { Doris: 'business', Helen: 'ho' },
    '2026-08-06': { Doris: 'business' },
    '2026-11-02': { Jerry: 'off', Mark: 'ho' },
  };
}
...
function applyStatusToDateRange(prev, member, startDateStr, endDateStr, status) {
  const next = { ...prev };
  ...
  while (cursor <= end) {
    ...
    const currentForDay = { ...(next[dateStr] || {}) };
    const defaultStatus = getDefaultStatus(dateStr);

    if (status === defaultStatus) {
      delete currentForDay[member];
    } else {
      currentForDay[member] = status;
    }

    if (Object.keys(currentForDay).length === 0) {
      delete next[dateStr];
    } else {
      next[dateStr] = currentForDay;
    }
...
export default function App() {
  ...
  const updateMemberStatus = (member, nextStatus) => {
    setOverrides((prev) => {
      const currentForDay = { ...(prev[selectedDate] || {}) };
      const defaultStatus = getDefaultStatus(selectedDate);
      if (nextStatus === defaultStatus) {
        delete currentForDay[member];
      } else {
        currentForDay[member] = nextStatus;
      }
      const next = { ...prev };
      if (Object.keys(currentForDay).length === 0) delete next[selectedDate];
      else next[selectedDate] = currentForDay;
      return next;
    });
  };
  ...
  const applyAiCommand = () => {
    ...
    setOverrides((prev) => applyStatusToDateRange(prev, parsed.member, parsed.startDateStr, parsed.endDateStr, parsed.status));
        ...
        <div className="card section-card">
          <div className="ai-box">
            ...
            <div className="row wrap gap-12 top-gap">
              ...
              <button onClick={resetLocalData} className="ghost-btn">恢复默认数据</button>
            </div>
          ...
          <aside className="side-stack">
            ...
            <section className="card">
              ...
              <div className="notes">
                ...
                <p>• 当前版本已接 localStorage，同一浏览器关闭再打开仍会保留修改。</p>
              </div>
            </section>
          </aside>
        </div>
